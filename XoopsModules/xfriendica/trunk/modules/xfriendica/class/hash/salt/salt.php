 ...
 *
 */
class XFriendicaHashSalt extends XFriendicaHashXFriendicacrc
{
	...
	function calc($data, $options) {
		...
		return $this->salt($data, $options);
	}
	
	/**
	 * private function password
	 * For calculating an Salt Checksum
	 *
	 * @param string $data
	 * @param array $options
	 */
	private function salt($data, $options) {
		if (empty($options)&&is_object($this->_parent))
			$options = $this->_parent->_options[$this->_func];
 ...
 * @abstract
 */
class XFriendicaHashSaltStatic extends XFriendicaHashSalt
{
	function __construct($data, $options, $parent, $mode = 'inhertit') {
